...
def detect_duplicates(invoices: list[InvoiceRecord]) -> list[InvoiceRecord]:
    ...
    by_invoice_number: dict[tuple[str, str, str], list[InvoiceRecord]] = defaultdict(list)
    by_vendor_amount_date: dict[tuple[str, str, float, str], list[InvoiceRecord]] = defaultdict(list)
    ...
    for invoice in updated:
        if invoice.invoice_number and (invoice.vendor_tax_id or invoice.vendor_name):
            ...
            by_invoice_number[
                (invoice.document_type, vendor_key, invoice.invoice_number.casefold())
            ].append(invoice)
        if invoice.vendor_name and invoice.total is not None and invoice.invoice_date:
            key = (
                invoice.document_type,
                invoice.vendor_name.casefold(),
                invoice.total,
